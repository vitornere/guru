...
def pergunta(request):
    if request.user.is_authenticated():
        perguntas = Pergunta.objects.filter(
            data_de_criacao__lte=timezone.now()).order_by('data_de_criacao')
    ...
    if request.user.is_authenticated():
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                comentario = pergunta.resposta_set.create(
                    autor=autor, texto=texto, data_de_criacao=data_de_criacao)
                comentario.save()
                form = ComentarioForm()
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                pergunta = Pergunta.objects.create(
                    autor=autor, titulo=titulo, texto=texto, data_de_criacao=data_de_criacao, data_de_fechamento=None)
                pergunta.save()
                form = ComentarioForm()
                return render(request, 'guru_responde/detalhe.html', {'pergunta': pergunta, 'form': form})
            else:
